 ...
 *
 *         The ProtocolException class is the superclass of all errors and exceptions in the Kura Protocol project. It
 *         extends
 *         the JDK Exception class by requesting its invokers to provide an error code when
 *         building its instances. The code is one value of ProtocolErrorCode enum; the code is used to document the
 *         possible
 *         error conditions generated by the platform as well as to identify the localized
 *         exception messages to be reported. Exceptions messages are stored in the ProtocolExceptionMessagesBundle
 *         Properties
